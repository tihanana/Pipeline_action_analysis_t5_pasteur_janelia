function trx = give_state_choreography_3_steps_all_at_once(trx, classifier, index_liste)
...
%     name_proba              = [name_state '_proba2'];    
    ...
    for i = 1 : length(trx)
        ...
        [~,m] = size(features);
        features_moins_1  = [nan(1,m);features(1:end-1,:)] ;
        features_plus_1   = [features(2:end,:);nan(1,m);] ;
        features_together = [features_moins_1, features, features_plus_1];
        features          = features_together(2:end-1,:);
        features          = [features(1,:); features  ; features(end,:)];
        clear features_together;
...
%         state_out       = sum(JJ,2);
        n_state_out            = length(state_out);
        global_state           = zeros(n_state_out,1);
        for j = 1 : n_state_out
            global_state(j,1)  = str2num(state_out{j});
        end
        trx(i).global_state = global_state;
        clear global_state n_state_out ;
